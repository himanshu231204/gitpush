...
@click.option(
    "-i", "--interactive", is_flag=True, help="Interactive mode - select commit to view details"
)
...
def show_rich_table(graph_data, all_branches):
    ...
    for i, commit in enumerate(graph_data):
        ...
        table.add_row(str(i + 1), hash_display, branch, msg, commit["author"][:15], date_str)
...
def display_commit_details(details):
    ...
    if details.get("diff") and details["diff"].get("files"):
        ...
        for f in details["diff"]["files"]:
            status_style = (
                "green" if f["status"] == "+" else "red" if f["status"] == "-" else "yellow"
            )
            table.add_row(
