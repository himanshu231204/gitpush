...
def new(repo_name, description, private, public, gitignore, license, no_readme, quick):
    ...
    if quick:
        ...
        if not private and not public:
            visibility = questionary.select("Repository visibility:", choices=['Public', 'Private']).ask()
        ...
        if not license:
            ...
            license_choice = questionary.select("Select license:", choices=list(licenses.keys())).ask()
